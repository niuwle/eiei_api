...
async def process_queue(
   ...
   try:
       ...
       await send_error_notification(
           chat_id,
           await get_bot_config(db, return_type="short_name", bot_id=bot_id),
           "Error: e001",
       )
   finally:
       await db.close()
...
async def process_message(
   ...
   for attempt in range(retries + 1):
       try:
           response_text = await asyncio.wait_for(
               get_chat_completion(chat_id, messages[0].bot_id, db), timeout=10
           )
           if response_text:
               break
       except asyncio.TimeoutError:
           logger.error(
               f"get_chat_completion timed out for chat_id {chat_id}, attempt {attempt + 1}"
           )
   ...
   if not response_text:
       logger.error(
           f"Failed to get chat completion response after {retries} retries for chat_id {chat_id}"
       )
       await send_error_notification(
           chat_id,
           await get_bot_config(db, return_type="short_name", bot_id=bot_id),
   ...
   if response_text:
       ...
       logger.debug(f"chat_id1234: {chat_id}")
           ...
           if success:
               ...
               if photo_temp_path:
                   await send_photo_message(
                       chat_id=chat_id, photo_temp_path=photo_temp_path, bot_token=bot_token
                   )
